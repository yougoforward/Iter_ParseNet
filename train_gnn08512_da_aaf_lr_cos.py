...
def adjust_learning_rate(optimizer, epoch, i_iter, iters_per_epoch, method='poly'):
    if method == 'poly':
        ...
        lr = cosine_decay(learning_rate=args.learning_rate, global_step=epoch * iters_per_epoch + i_iter, warm_step = 5 * iters_per_epoch, warm_lr=0.1 * args.learning_rate, decay_steps = args.epochs * iters_per_epoch, alpha=0.0001)
    elif method== 'restart_cosine':
        lr = restart_cosine_decay(learning_rate=args.learning_rate, global_step=epoch * iters_per_epoch + i_iter, warm_step = 5 * iters_per_epoch, warm_lr=0.1 * args.learning_rate, decay_steps = args.epochs * iters_per_epoch, alpha=0.0001)
    else:
