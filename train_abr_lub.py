...
from dataset.dataloader import lub_DataGenerator as DataGenerator
# from dataset.datasets import DatasetGenerator
from network.abrnet_lub import get_model
# from network.abrnet import get_model
from progress.bar import Bar
from utils.lovasz_loss import lub_ABRLovaszLoss as ABRLovaszLoss
    ...
    for idx, batch in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            bar.suffix = '{} / {} | pixAcc: {pixAcc:.4f}, mIoU: {IoU:.4f} |'.format(idx + 1, len(val_loader),
                                                                                    pixAcc=pixAcc, IoU=IoU)
            bar.next()
    per_class_iou = per_class_iu(hist) * 100
    print('per class iou: {}'.format(per_class_iou))
