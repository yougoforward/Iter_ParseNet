...
class Decoder(nn.Module):
    def __init__(self, num_classes):
        ...
        # self.W = nn.Conv2d(num_classes, 64*num_classes, kernel_size=1, padding=0, stride=1, bias=False)
        # self.P = nn.Conv2d(64*num_classes, num_classes, kernel_size=1, padding=0, stride=1, bias=False)
        # nn.init.orthogonal_(self.W.weight)
        # nn.init.orthogonal_(self.P.weight)

        self.W = nn.Parameter(torch.zeros(num_classes, 64*num_classes))
        self.P = nn.Parameter(torch.zeros(64*num_classes, num_classes))
    ...
    def forward(self, x):
        ...
        # x_seg = self.shuffle(self.W(x_seg))

        # bs, c, h, w = x_seg.size()
        x_seg = x_seg.permute(0,2,3,1) # bs, h, w, c
        x_seg_ex = torch.matmul(x_seg,self.W)
        x_seg_ex=x_seg_ex.permute(0,3,1,2)
        x_seg = self.shuffle(x_seg_ex)

        return [x_seg, alpha_hb, alpha_fb, self.W, self.P, x_dsn]
