...
class Trans_infer_rnn_tree(nn.Module):
    def __init__(self, cls_p=7, cls_h=3, cls_f=2, half_upper_nodes=[1, 2, 3, 4], half_lower_nodes=[5, 6]):
        ...
        self.td_half_upper_rnn = Seq_ConvLSTMCell(input_dim=256 + 1, hidden_dim=10, kernel_size=(1, 1), bias=False)
        self.td_half_lower_rnn = Seq_ConvLSTMCell(input_dim=256 + 1, hidden_dim=10, kernel_size=(1, 1), bias=False)
        self.td_part_leaf_list = nn.ModuleList(
            [Seq_ConvLSTMCell(input_dim=256 + 1, hidden_dim=10, kernel_size=(1, 1), bias=False) for i in
             range(0, cls_p - 1)])
