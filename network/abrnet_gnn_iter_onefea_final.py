...
class GNN(nn.Module):
    def __init__(self, in_dim=256, hidden_dim=10, cls_p=7, cls_h=3, cls_f=2):
        ...
        self.full_decomp = nn.Sequential(
            nn.Conv2d(in_dim + hidden_dim, 256, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(256), nn.ReLU(inplace=False),
            nn.Conv2d(256, (cls_h-1) * hidden_dim, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d((cls_h-1) * hidden_dim), nn.ReLU(inplace=False)
        )
        self.upper_half_decomp = nn.Sequential(
            nn.Conv2d(in_dim + hidden_dim, 256, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(256), nn.ReLU(inplace=False),
            nn.Conv2d(256, self.upper_node_len * hidden_dim, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(self.upper_node_len * hidden_dim), nn.ReLU(inplace=False)
        )
        self.lower_half_decomp = nn.Sequential(
            nn.Conv2d(in_dim + hidden_dim, 256, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(256), nn.ReLU(inplace=False),
            nn.Conv2d(256, self.lower_node_len * hidden_dim, kernel_size=1, padding=0, stride=1, bias=False),
    ...
    def forward(self, xp_list, xh_list, xf, bg_node, p_fea, h_fea, f_fea):
        ...
        # for half body node
        xfh_list = list(torch.split(self.full_decomp(torch.cat([h_fea, xf], dim=1)), self.hidden, dim=1))
        ...
        #upper half nodes
        upper_xhp_list = list(torch.split(self.upper_half_decomp(torch.cat([p_fea, xh_list[0]],dim=1)), self.hidden,dim=1))
        # lower half nodes
        lower_xhp_list = list(
            torch.split(self.lower_half_decomp(torch.cat([p_fea, xh_list[1]], dim=1)), self.hidden, dim=1))
