...
class DecoderModule(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.conv0 = nn.Sequential(nn.Conv2d(64, 512, kernel_size=3, padding=1, dilation=1, bias=False),
                                   BatchNorm2d(512), nn.ReLU(inplace=False))
        self.conv1 = nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, padding=1, dilation=1, bias=False),
                                   BatchNorm2d(256), nn.ReLU(inplace=False))

        self.conv2 = nn.Sequential(nn.Conv2d(32, 48, kernel_size=1, stride=1, padding=0, dilation=1, bias=False),
    ...
    def forward(self, xt, xm, xl):
        _, _, h, w = xm.size()
        xt = F.interpolate(xt, size=(h, w), mode='bilinear', align_corners=True) + self.alpha * self.conv0(xm)
        _, _, th, tw = xl.size()
...
class AlphaHBDecoder(nn.Module):
    ...
    def forward(self, x, skip):
        ...
        xfuse = xup + self.alpha_hb * self.conv0(skip)
...
class AlphaFBDecoder(nn.Module):
    ...
    def forward(self, x, skip):
        ...
        xup = F.interpolate(x, size=(h, w), mode='bilinear', align_corners=True)
        xfuse = xup + self.alpha_fb * self.conv0(skip)
        output = self.conv1(xfuse)
        return output
class Decoder(nn.Module):
    def __init__(self, num_classes):
        super(Decoder, self).__init__()
        self.layer5 = MagicModule(480, 512, 1)
    ...
    def forward(self, x):
        ...
        _, _, h, w = x[-2].size()
        x4 = F.interpolate(x[-1], (h, w), mode='bilinear', align_corners=True)
        x2 = F.interpolate(x[-3], (h, w), mode='bilinear', align_corners=True)
        x1 = F.interpolate(x[-4], (h, w), mode='bilinear', align_corners=True)
        x[-2] = torch.cat((x1,x2, x[-2], x4), dim=1)
        seg = self.layer5(x[-2])
