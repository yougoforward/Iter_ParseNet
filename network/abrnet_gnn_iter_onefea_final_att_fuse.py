...
class GNN_infer(nn.Module):
    def __init__(self, in_dim=256, hidden_dim=10, cls_p=7, cls_h=3, cls_f=2):
        ...
        self.p_dconv = nn.Sequential(
            nn.Conv2d(hidden_dim * (cls_p+cls_h+cls_f-2), in_dim, kernel_size=1, padding=0, stride=1, bias=False),
    ...
    def forward(self, xp, xh, xf):
        ...
        #feature d_transform
        #feature d_transform
        xp_infer = xp+self.alpha_p*self.p_dconv(torch.cat([bg_node]+p_fea_list_new + h_fea_list_new+ [f_fea_new], dim=1))
        xh_infer = xh+self.alpha_h*self.h_dconv(torch.cat([bg_node]+p_fea_list_new + h_fea_list_new+ [f_fea_new], dim=1))
        xf_infer = xf+self.alpha_f*self.f_dconv(torch.cat([bg_node]+p_fea_list_new + h_fea_list_new+ [f_fea_new], dim=1))
