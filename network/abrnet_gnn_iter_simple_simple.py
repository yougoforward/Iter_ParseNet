...
class Part_Graph(nn.Module):
    ...
    def forward(self, p_fea, xp_list, xhp_list):
        ...
        xp_list = torch.cat(xp_list, dim=1)+xhp_list
        return xp_list
...
class GNN(nn.Module):
    def __init__(self, in_dim=256, hidden_dim=10, cls_p=7, cls_h=3, cls_f=2):
        ...
        self.full_update = nn.Sequential(
            nn.Conv2d(in_dim + cls_f*hidden_dim, cls_f * hidden_dim, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(cls_f * hidden_dim), nn.ReLU(inplace=False)
        )
        self.half_update = nn.Sequential(
            nn.Conv2d(in_dim + cls_h*hidden_dim, cls_h * hidden_dim, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(cls_h * hidden_dim), nn.ReLU(inplace=False),
        )
        self.part_update = nn.Sequential(
            nn.Conv2d(in_dim + cls_p * hidden_dim, cls_p * hidden_dim, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(cls_p * hidden_dim), nn.ReLU(inplace=False),
        )
    ...
    def forward(self, xp_list, xh_list, xf_list, p_fea, h_fea, f_fea):
        ...
        xf_list_new = torch.split(self.full_update(torch.cat([f_fea, xf_list[0],xf_new], dim=1)), self.hidden, dim=1)
        ...
        xh_list_new = torch.split(self.half_update(torch.cat([h_fea, xh_list[0]+xfh_list[0], xhu_new, xhl_new], dim=1)), self.hidden, dim=1)
        ...
        xhp_list = self.half_decomp(torch.cat([p_fea, xh_list[1], xh_list[2]], dim=1))
        xp_list_new = self.part_infer(p_fea, xp_list, xhp_list)
        xp_list_new = torch.split(self.part_update(torch.cat([p_fea, xp_list_new], dim=1)), self.hidden, dim=1)
