...
class Half_upper_Graph(nn.Module):
    def __init__(self, part_list=[1,2,3,4], in_dim=256, hidden_dim=10, cls_p=7, cls_h=3, cls_f=2):
        ...
        self.conv_ph = nn.Sequential(
            nn.Conv2d(self.parts * hidden_dim, 32, kernel_size=3, padding=1, stride=1, bias=False),
            BatchNorm2d(32), nn.ReLU(inplace=False),
            nn.Conv2d(32, hidden_dim, kernel_size=3, padding=1, stride=1, bias=False),
            BatchNorm2d(hidden_dim), nn.ReLU(inplace=False)
        )
    ...
    def forward(self,h_fea, xh_list, xfh_att, xp_list):
        ...
        xfh = self.conv_fh(xfh_att*h_fea)
        message = xph + xlh + xfh
...
class GNN(nn.Module):
    ...
    def forward(self, xp_list, xh_list, xf, bg_node, p_fea, h_fea, f_fea):
        ...
        xhu_new, xhu_message = self.half_upper_infer(h_fea, xh_list, xfh_att_list[0], xp_list)
        xhl_new, xhl_message = self.half_lower_infer(h_fea, xh_list, xfh_att_list[1], xp_list)
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # h_seg = self.fuse_h([alpha_hb, xh_seg, node_h_seg])Final_classifer
