...
class Trans_infer_half(nn.Module):
    def __init__(self, part_cls, half_cls, full_cls):
        super(Trans_infer_half, self).__init__()

        self.conv1 = nn.Sequential(nn.Conv2d(part_cls+half_cls+full_cls, 32, kernel_size=1, padding=0, stride=1, bias=False),
                                   BatchNorm2d(32), nn.ReLU(inplace=False),
                                   nn.Conv2d(32, 32, kernel_size=1, padding=0, stride=1, bias=False),
                                   BatchNorm2d(32), nn.ReLU(inplace=False)
                                   )
        ...
        self.conv3 = nn.Sequential(nn.Conv2d(96, 64, kernel_size=1, padding=0, stride=1, bias=False),
                                   BatchNorm2d(64), nn.ReLU(inplace=False),
                                   nn.Conv2d(64, half_cls, kernel_size=1, padding=0, stride=1, bias=True))

    def forward(self, x_part, x_half, x_full, x_fea):
        x = torch.cat([x_part, x_half, x_full], dim=1)
        x = self.conv1(x)
...
class Trans_infer_half_rnn_ed(nn.Module):
    def __init__(self, part_cls, half_cls, full_cls, rnn_type='RNN'):
        super(Trans_infer_half_rnn_ed, self).__init__()
        self.rnn_type = rnn_type
        if self.rnn_type == 'rnn':
            self.rnn = convolutional_rnn.Conv2dRNN(in_channels=128, out_channels=32,
                                                   kernel_size=1,
                                                   num_layers=1,
                                                   bias=True,
                                                   batch_first=False,
                                                   ...
                                                   groups=1)
        if self.rnn_type == 'gru':
            self.rnn = convolutional_rnn.Conv2dGRU(in_channels=128, out_channels=32,
                                                   kernel_size=1,
                                                   num_layers=1,
                                                   bias=True,
                                                   batch_first=False,
                                                   ...
                                                   groups=1)
        if self.rnn_type == 'lstm':
            self.rnn = convolutional_rnn.Conv2dLSTM(in_channels=128, out_channels=32,
                                                    kernel_size=1,
                                                    num_layers=1,
                                                    bias=True,
                                                    batch_first=False,
        ...
        self.conv1_1 = nn.Sequential(nn.Conv2d(part_cls, 32, kernel_size=1, padding=0, stride=1, bias=False),
                                   BatchNorm2d(32), nn.ReLU(inplace=False),
                                   nn.Conv2d(32, 32, kernel_size=1, padding=0, stride=1, bias=False),
                                   BatchNorm2d(32), nn.ReLU(inplace=False)
                                   )
        self.conv1_2 = nn.Sequential(nn.Conv2d(half_cls, 32, kernel_size=1, padding=0, stride=1, bias=False),
                                   BatchNorm2d(32), nn.ReLU(inplace=False),
                                   nn.Conv2d(32, 32, kernel_size=1, padding=0, stride=1, bias=False),
                                   BatchNorm2d(32), nn.ReLU(inplace=False)
                                   )
        self.conv1_3 = nn.Sequential(nn.Conv2d(full_cls, 32, kernel_size=1, padding=0, stride=1, bias=False),
                                     BatchNorm2d(32), nn.ReLU(inplace=False),
                                     nn.Conv2d(32, 32, kernel_size=1, padding=0, stride=1, bias=False),
                                     BatchNorm2d(32), nn.ReLU(inplace=False)
                                     )
        self.conv3 = nn.Sequential(nn.Conv2d(32, 32, kernel_size=1, padding=0, stride=1, bias=False),
                                   BatchNorm2d(32), nn.ReLU(inplace=False),
                                   nn.Conv2d(32, half_cls, kernel_size=1, padding=0, stride=1, bias=True))


    def forward(self, x_part, x_half, x_full, x_fea):

        x_part=self.conv1_1(x_part)
        x_half=self.conv1_2(x_half)
        x_full = self.conv1_3(x_full)

        x_fea = self.conv2(x_fea)
        x_fuse = torch.cat([x_part, x_full, x_fea], dim=1)

        _, x_fine = self.rnn(x_fuse.unsqueeze(0), x_half.unsqueeze(0))
        x_fine = self.conv3(x_fine[0])
...
class Trans_infer_half_rnn(nn.Module):
    def __init__(self, part_cls, half_cls, full_cls, rnn_type='RNN'):
        super(Trans_infer_half_rnn, self).__init__()
        self.rnn_type=rnn_type
        if self.rnn_type=='rnn':
            self.net = convolutional_rnn.Conv2dRNN(in_channels=64 + part_cls+full_cls, out_channels=half_cls,
                                                   kernel_size=1,
                                                   num_layers=1,
                                                   bias=True,
                                                   batch_first=False,
                                                   ...
                                                   groups=1)
        if self.rnn_type=='gru':
            self.net = convolutional_rnn.Conv2dGRU(in_channels=64 + part_cls+full_cls, out_channels=half_cls,
                                                   kernel_size=1,
                                                   num_layers=1,
                                                   bias=True,
                                                   batch_first=False,
                                                   ...
                                                   groups=1)
        if self.rnn_type=='lstm':
            self.net = convolutional_rnn.Conv2dLSTM(in_channels=64 + part_cls+full_cls, out_channels=half_cls,
                                                   kernel_size=1,
                                                   num_layers=1,
                                                   bias=True,
                                                   batch_first=False,
    ...
    def forward(self, x_part, x_half, x_full, x_fea):
        ...
        _, x_fine = self.net(x_fuse.unsqueeze(0), x_half.unsqueeze(0))
...
class IterTrans_iccv(nn.Module):
    def __init__(self, trans_step=2, trans_unit='rnn', fbody_cls=2, hbody_cls=3, part_cls=7):
        super(IterTrans_iccv, self).__init__()
        self.trans_step = trans_step
        self.trans_unit = trans_unit
        if self.trans_unit == "conv":
            self.PartFullHalfInfer = Trans_infer_half(part_cls, hbody_cls, fbody_cls)
            self.HalfFullInfer = Trans_infer(hbody_cls, fbody_cls)
            self.HalfPartInfer = Trans_infer(hbody_cls, part_cls)
        if self.trans_unit in ["rnn", 'lstm', 'gru']:
            # self.PartFullHalfInfer = Trans_infer_half_rnn(part_cls, hbody_cls, fbody_cls, self.trans_unit)
            # self.HalfFullInfer = Trans_infer_rnn(hbody_cls, fbody_cls, self.trans_unit)
            # self.HalfPartInfer = Trans_infer_rnn(hbody_cls, part_cls, self.trans_unit)

            self.PartFullHalfInfer = Trans_infer_half_rnn_ed(part_cls, hbody_cls, fbody_cls, self.trans_unit)
            self.HalfFullInfer = Trans_infer_rnn_ed(hbody_cls, fbody_cls, self.trans_unit)
            self.HalfPartInfer = Trans_infer_rnn_ed(hbody_cls, part_cls, self.trans_unit)


    def forward(self, x_part, x_hbody, x_fbody, p_fea, h_fea, f_fea):
        x_hbody_new = self.PartFullHalfInfer(x_part, x_hbody, x_fbody, h_fea)
        x_fbody_new = self.HalfFullInfer(x_hbody_new, x_fbody, f_fea)
        x_part_new = self.HalfPartInfer(x_hbody_new, x_part, p_fea)

        return x_part_new, x_hbody_new, x_fbody_new

class Decoder(nn.Module):
    def __init__(self, num_classes=7, iter_step=2, trans_step=1, fbody_cls=2, hbody_cls=3):
        ...
        self.trans_step = trans_step
        self.iter_trans = IterTrans_iccv(self.trans_step, 'rnn', fbody_cls, hbody_cls, part_cls=num_classes)
