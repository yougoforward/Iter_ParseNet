...
class GamaHBDecoder(nn.Module):
    def __init__(self, fbody_cls, hbody_cls):
        ...
        self.gate = GatingBlock(in_dim=fbody_cls, out_dim=fbody_cls, force_hard=True)
        self.conv1 = nn.Sequential(nn.Conv2d(fbody_cls, 32, kernel_size=1, padding=0, stride=1, bias=False),
                                   ...
                                   InPlaceABNSync(32),
                                   nn.Conv2d(32, hbody_cls, kernel_size=1, padding=0, stride=1, bias=True))

    def forward(self, x):
        x, act_ghb = self.gate(x)
        output = self.conv1(x)
        return output, act_ghb
...
class GamaPartDecoder(nn.Module):
    ...
    def forward(self, x):
        x, act_gp = self.gate(x)
        output = self.conv1(x)
        return output, act_gp
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        gama_hb, act_ghb = self.layergh(alpha_fb)
        gama_part, act_gp = self.layergp(alpha_hb)
        ...
        for i in range(self.iter_step):
            x_seg, x_hbody, x_fbody, xp, xh, xf = self.iter_trans(x_seg, x_hbody, x_fbody, xp, xh, xf)

        return [x_part, gama_part, x_hb, beta_hb, gama_hb, x_fb, beta_fb, gate_acts, x_seg, alpha_hb, alpha_fb]
