...
class Full_Graph(nn.Module):
    ...
    def forward(self, f_fea, xf, xh):
        message=self.conv_hf(torch.cat(xh, dim=1))
        # xf = self.conv_update(torch.cat([xf, message], dim=1))

        att = self.attention(torch.cat([xf, message], dim=1))
        xf = self.conv_update(att*f_fea)
        return xf, message
...
class Half_upper_Graph(nn.Module):
    ...
    def forward(self, h_fea, xh_list, xfh, xp_list):
        ...
        att = self.attention(torch.cat([xh_list[0], message], dim=1))
        xh_u = self.conv_update(att*h_fea)
...
class Half_lower_Graph(nn.Module):
    ...
    def forward(self, h_fea, xh_list, xfh, xp_list):
        ...
        # xh_u = self.conv_update(torch.cat([xh_list[0], message], dim=1))
        att = self.attention(torch.cat([xh_list[1], message], dim=1))
        xh_l = self.conv_update(att * h_fea)
        return xh_l, message
...
class Part_Graph(nn.Module):
    def __init__(self, in_dim=256, hidden_dim=10, cls_p=7, cls_h=3, cls_f=2):
        ...
        self.attention_list = nn.ModuleList([nn.Sequential(
            nn.Conv2d(2 * hidden_dim, 32, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(32), nn.ReLU(inplace=False),
            nn.Conv2d(32, 32, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(32), nn.ReLU(inplace=False),
        ...
        self.update_conv_list = nn.ModuleList([ nn.Sequential(
            nn.Conv2d(in_dim, hidden_dim, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(hidden_dim), nn.ReLU(inplace=False)
        ) for i in range(cls_p-1)])
    ...
    def forward(self, p_fea, xp_list, xhp_list):
        ...
        att_list = [
            self.attention_list[j](torch.cat([xp_list[j], message_list[j]], dim=1))
            for j in range(self.cls_p - 1)]
        xp_list = [self.update_conv_list[j](p_fea*att_list[j])
            for j in range(self.cls_p - 1)]
...
class GNN(nn.Module):
    ...
    def forward(self, xp_list, xh_list, xf, bg_node, p_fea, h_fea, f_fea):
        # for full body node
        xf_new, xf_message = self.full_infer(f_fea, xf, xh_list)
