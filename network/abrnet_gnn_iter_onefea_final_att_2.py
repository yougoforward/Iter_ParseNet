...
class Full_Graph(nn.Module):
    ...
    def forward(self, xf, xh):
        message=self.conv_hf(torch.cat([xh[1],xh[2]], dim=1))
        xf = self.conv_update(torch.cat([xf[1], message], dim=1))
...
class Half_upper_Graph(nn.Module):
    ...
    def forward(self,h_fea, xh_list, xfh, xp_list):
        ...
        xlh = self.conv_lh(torch.cat([h_fea, xh_list[2]], dim=1))
        message = xph + xlh + xfh
        xh_u = self.conv_update(torch.cat([xh_list[1], message], dim=1))
        return xh_u, message
...
class Half_lower_Graph(nn.Module):
    ...
    def forward(self, h_fea, xh_list, xfh, xp_list):
        ...
        xph = self.conv_ph(torch.cat(parts, dim=1))
        xuh = self.conv_lh(torch.cat([h_fea, xh_list[1]], dim=1))
        message = xph+xuh+xfh
        xh_l = self.conv_update(torch.cat([xh_list[2], message], dim=1))
...
class Part_Graph(nn.Module):
    ...
    def forward(self, p_fea, xp_list, xhp_list):
        ...
        xpp_list = [self.node_conv_list[j](torch.cat([p_fea]+[xp_list[i+1] for i in range(self.cls_p-1) if i!=j], dim=1)) for j in range(self.cls_p-1)]
        message_list = [xpp_list[j]+xhp_list[j] for j in range(self.cls_p - 1)]
        xp_list_new = [
            self.update_conv_list[j](torch.cat([xp_list[j+1], message_list[j]], dim=1))
            for j in range(self.cls_p - 1)]
        return xp_list_new, message_list
...
class GNN(nn.Module):
    ...
    def forward(self, xp_list, xh_list, xf_list, p_fea, h_fea, f_fea):
        # for full body node
        xf_new, xf_message = self.full_infer(xf_list, xh_list)
        xf_list_new = [xf_list[0], xf_new]
        # for half body node
        xfh_list = list(torch.split(self.full_decomp(h_fea*self.full_att(xf_list[1])), self.hidden, dim=1))
        ...
        xh_list_new = [xh_list[0], xhu_new, xhl_new]
        ...
        #upper half nodes
        upper_xhp_list = list(torch.split(self.upper_half_decomp(p_fea*self.upper_att(xh_list[1])), self.hidden,dim=1))
        # lower half nodes
        lower_xhp_list = list(
            torch.split(self.lower_half_decomp(p_fea*self.lower_att(xh_list[2])), self.hidden, dim=1))
        ...
        xp_list_new = [xp_list[0]]+xp_list_new

        return xp_list_new, xh_list_new, xf_list_new, xf_message, xh_message_list, xp_message_list
...
class GNN_infer(nn.Module):
    ...
    def forward(self, xp, xh, xf):
        ...
        f_node_list = list(torch.split(self.f_conv(xh), self.hidden, dim=1))
        ...
        p_fea_list_new, h_fea_list_new, f_fea_list_new, f_message, h_message_list, p_message_list = self.gnn(p_node_list, h_node_list, f_node_list, xp, xh, xf)


        #node supervision
        pg_seg = self.pg_cls(torch.cat(p_node_list, dim=1))
        hg_seg = self.hg_cls(torch.cat(h_node_list, dim=1))
        fg_seg = self.fg_cls(torch.cat(f_node_list, dim=1))

        f_message_seg = self.f_message_cls(torch.cat([f_node_list[0], f_message], dim=1))
        h_message_seg = self.h_message_cls(torch.cat([h_node_list[0]]+h_message_list, dim=1))
        p_message_seg = self.p_message_cls(torch.cat([p_node_list[0]]+p_message_list, dim=1))
        ...
        #feature d_transform
        #feature d_transform
        xp_infer = xp+self.alpha_p*self.p_dconv(torch.cat(p_fea_list_new, dim=1))
        xh_infer = xh+self.alpha_h*self.h_dconv(torch.cat(h_fea_list_new, dim=1))
        xf_infer = xf+self.alpha_f*self.f_dconv(torch.cat(f_fea_list_new, dim=1))
