...
class EfficientNet(nn.Module):
    ...
    def extract_features(self, inputs):
        ...
        for idx, block in enumerate(self._blocks):
            ...
            if len(self._blocks)==16 and idx in [2, 4, 10, 15]:
                out.append(x)
            if len(self._blocks)==23 and idx in [4,7,15,22]:
                out.append(x)
            if len(self._blocks)==26 and idx in [4,7,17,25]:
                out.append(x)
            if len(self._blocks)==32 and idx in [5,9,21,31]:
                out.append(x)
            if len(self._blocks) == 39 and idx in [7, 12, 26, 38]:
                out.append(x)
    ...
    def forward(self, inputs):
        ...
        # Convolution layers
        x, out = self.extract_features(inputs)
