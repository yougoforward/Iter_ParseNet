...
class Final_DecoderModule(nn.Module):
    ...
    def __init__(self, cls_p=7, cls_h=3, cls_f=2):
        ...
        self.conv3 = nn.Sequential(nn.Conv2d(108, 256, kernel_size=1, padding=0, dilation=1, bias=False),
                                   BatchNorm2d(256), nn.ReLU(inplace=False),
                                   nn.Conv2d(256, 256, kernel_size=1, padding=0, dilation=1, bias=False),
                                   BatchNorm2d(256), nn.ReLU(inplace=False))
        ...
        self.conv7 = nn.Conv2d(20, cls_h, kernel_size=1, padding=0, dilation=1, bias=True)
        self.conv8 = nn.Conv2d(10, cls_f, kernel_size=1, padding=0, dilation=1, bias=True)
...
class Trans_infer_rnn_tree(nn.Module):
    def __init__(self, cls_p=7, cls_h=3, cls_f=2, half_upper_nodes=[1, 2, 3, 4], half_lower_nodes=[5, 6]):
        ...
        self.part_leaf_list = nn.ModuleList(
            [leaf_ConvLSTMCell(input_dim=256, hidden_dim=10, kernel_size=(3, 3), bias=False) for i in
             range(0, cls_p - 1)])
...
class IterTrans(nn.Module):
    ...
    def forward(self, x_part, x_hbody, x_fbody, p_fea, h_fea, f_fea):
        ...
        part_fea = torch.cat(part_h_list, dim=1)
        half_fea = torch.cat(half_h_list, dim=1)


        return part_fea, half_fea, full_h, part_h_list, half_h_list
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # x_fea1, hb_fea1, fb_fea1, p_fea_list, h_fea_list = self.iter_trans(x_seg, hb_seg, fb_seg, x_fea, hb_fea, fb_fea)

        # x_seg1, hb_seg1, fb_seg1 = self.final(x_fea1, hb_fea1, fb_fea1, x[0])
        # p_seg_list, h_seg_list = self.node_seg(p_fea_list, h_fea_list)

        # return [x_seg1, hb_seg1, fb_seg1, x_seg, hb_seg, fb_seg, p_seg_list, h_seg_list, x_dsn]

        return [x_seg, hb_seg, fb_seg, x_dsn]
