...
from modules.jpu_mod import simple_JPU
...
class Decoder(nn.Module):
    def __init__(self, num_classes):
        ...
        self.simple_jpu = simple_JPU(512, 256, BatchNorm2d)
    ...
    def forward(self, x):
        ...
        seg = self.layer5(x[-1])
        seg = self.simple_jpu(x[1], seg)
        x_seg, x_fea = self.layer6(seg, x[0])
        alpha_hb = self.layerh(seg)
        alpha_fb = self.layerf(seg)
