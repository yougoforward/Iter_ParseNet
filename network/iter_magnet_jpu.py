...
class ResGridNet(nn.Module):
    ...
    def __init__(self, block, layers):
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=1, dilation=1, multi_grid=True)
...
class Trans_infer_rnn_ed(nn.Module):
    def __init__(self, num_classes1, num_classes2, rnn_type='RNN'):
        ...
        if self.rnn_type == 'rnn':
            self.net = convolutional_rnn.Conv2dRNN(in_channels=352, out_channels=32,
                                                   kernel_size=1,
                                                   num_layers=1,
                                                   bias=True,
                                                   batch_first=False,
        ...
        if self.rnn_type == 'gru':
            self.net = convolutional_rnn.Conv2dGRU(in_channels=352, out_channels=32,
                                                   kernel_size=1,
                                                   num_layers=1,
                                                   bias=True,
                                                   batch_first=False,
        ...
        if self.rnn_type == 'lstm':
            self.net = convolutional_rnn.Conv2dLSTM(in_channels=352, out_channels=32,
                                                    kernel_size=1,
                                                    num_layers=1,
                                                    bias=True,
                                                    batch_first=False,
    ...
    def forward(self, x, x_coarse, x_fea, x_coarse_fea):
        ...
        x_fea = self.conv2(x_fea)
        x_fuse = torch.cat([x, x_fea, x_coarse_fea], dim=1)

        _, x_fine = self.net(x_fuse.unsqueeze(0).repeat(self.ts,1,1,1,1), x_coarse.unsqueeze(0))
...
class Trans_infer_rnn_fea_s(nn.Module):
    def __init__(self, num_classes1, num_classes2, rnn_type='RNN'):
        super(Trans_infer_rnn_fea_s, self).__init__()
        self.ts=2
        self.rnn_type = rnn_type
        self.in_dim = 64+num_classes1-1+num_classes2-1
        if self.rnn_type == 'rnn':
            self.net = convolutional_rnn.Conv2dRNN(in_channels=self.in_dim, out_channels=256,
                                                   kernel_size=1,
                                                   num_layers=1,
                                                   bias=True,
                                                   batch_first=False,
                                                   ...
                                                   groups=1)
        if self.rnn_type == 'gru':
            self.net = convolutional_rnn.Conv2dGRU(in_channels=self.in_dim, out_channels=256,
                                                   kernel_size=1,
                                                   num_layers=1,
                                                   bias=True,
                                                   batch_first=False,
                                                   ...
                                                   groups=1)
        if self.rnn_type == 'lstm':
            self.net = convolutional_rnn.Conv2dLSTM(in_channels=self.in_dim, out_channels=256,
                                                    kernel_size=1,
                                                    num_layers=1,
                                                    bias=True,
                                                    batch_first=False,
    ...
    def forward(self, x, x_coarse, x_fea, x_coarse_fea):
        ...
        x_fea = self.conv2(x_fea)
        x_fuse = torch.cat([x[:,1:,:,:], x_coarse[:,1:,:,:], x_fea], dim=1)
...
class IterTrans(nn.Module):
    def __init__(self, trans_step=2, trans_unit='rnn', fbody_cls=2, hbody_cls=3, part_cls=7):
        ...
        if self.trans_unit in ["rnn", 'lstm', 'gru']:
            ...
            self.PartHalfInfer = Trans_infer_rnn_fea_s(part_cls, hbody_cls, self.trans_unit)
            self.HalfFullInfer = Trans_infer_rnn_fea_s(hbody_cls, fbody_cls, self.trans_unit)
            self.FullHalfInfer = Trans_infer_rnn_fea_s(fbody_cls, hbody_cls, self.trans_unit)
            self.HalfPartInfer = Trans_infer_rnn_fea_s(hbody_cls, part_cls, self.trans_unit)
    ...
    def forward(self, x_part, x_hbody, x_fbody, p_fea, h_fea, f_fea):
        ...
        # x_hbody = self.PartHalfInfer(x_part, x_hbody, p_fea, h_fea)
        # x_fbody = self.HalfFullInfer(x_hbody, x_fbody, h_fea, f_fea)
        # x_hbody = self.FullHalfInfer(x_fbody, x_hbody, f_fea, h_fea)
        # x_part = self.HalfPartInfer(x_hbody, x_part, h_fea, p_fea)
...
class Decoder(nn.Module):
    def __init__(self, num_classes):
        ...
        self.layer7 = DecoderModule(num_classes)
    ...
    def forward(self, x):
        ...
        x_seg2, alpha_hb2, alpha_fb2, x_fea2, hb_fea2, fb_fea2 = self.iter_trans(x_seg1, alpha_hb1, alpha_fb1, x_fea1, hb_fea1,
                                                                           fb_fea1)
        x_seg_final, _ = self.layer7(x_fea2, x[1], x[0])
