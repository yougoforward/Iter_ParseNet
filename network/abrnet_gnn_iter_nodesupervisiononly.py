...
class GNN(nn.Module):
    def __init__(self, in_dim=256, hidden_dim=10, cls_p=7, cls_h=3, cls_f=2):
        ...
        self.full_infer = Full_Graph(in_dim=in_dim, hidden_dim=hidden_dim, cls_p=cls_p, cls_h=cls_h, cls_f=cls_f)
        self.half_upper_infer = Half_upper_Graph(part_list=[1,2,3,4], in_dim=in_dim, hidden_dim=hidden_dim, cls_p=cls_p, cls_h=cls_h, cls_f=cls_f)
        self.half_lower_infer = Half_lower_Graph(part_list=[5,6], in_dim=in_dim, hidden_dim=hidden_dim, cls_p=cls_p, cls_h=cls_h, cls_f=cls_f)
        self.part_infer = Part_Graph(in_dim=in_dim, hidden_dim=hidden_dim, cls_p=cls_p, cls_h=cls_h, cls_f=cls_f)
