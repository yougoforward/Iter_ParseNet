...
class Half_upper_Graph(nn.Module):
    def __init__(self, part_list=[1,2,3,4], in_dim=256, hidden_dim=10, cls_p=7, cls_h=3, cls_f=2):
        ...
        self.conv_update = nn.Sequential(
            nn.Conv2d(in_dim + 4*hidden_dim, 2 * hidden_dim, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(2 * hidden_dim), nn.ReLU(inplace=False),
            nn.Conv2d(2 * hidden_dim, hidden_dim, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(hidden_dim), nn.ReLU(inplace=False)
        )
...
class Part_Graph(nn.Module):
    def __init__(self, in_dim=256, hidden_dim=10, cls_p=7, cls_h=3, cls_f=2):
        ...
        self.update_conv_list = nn.ModuleList(
            [nn.Sequential(
            nn.Conv2d(in_dim + 3*hidden_dim, 2 * hidden_dim, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(2 * hidden_dim), nn.ReLU(inplace=False),
            nn.Conv2d(2 * hidden_dim, hidden_dim, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(hidden_dim), nn.ReLU(inplace=False)
        ) for i in range(cls_p-1)])
    ...
    def forward(self, p_fea, xp_list, xhp_list):
        ...
        xp_list = [
            self.update_conv_list[j](torch.cat([p_fea, xp_list[j+1], xpp_list[j], xhp_list[j]], dim=1))
...
class GNN(nn.Module):
    def __init__(self, in_dim=256, hidden_dim=10, cls_p=7, cls_h=3, cls_f=2):
        ...
        self.full_decomp = nn.Sequential(
            nn.Conv2d(in_dim + hidden_dim, in_dim, kernel_size=1, padding=0, stride=1, bias=False),
        ...
        self.half_decomp = nn.Sequential(
            nn.Conv2d(in_dim + 2*hidden_dim, in_dim, kernel_size=1, padding=0, stride=1, bias=False),
            BatchNorm2d(in_dim), nn.ReLU(inplace=False),
            nn.Conv2d(in_dim, (cls_p-1)*hidden_dim, kernel_size=1, padding=0, stride=1, bias=False),
    ...
    def forward(self, xp_list, xh_list, xf_list, p_fea, h_fea, f_fea):
        ...
        xhu_new = self.half_upper_infer(h_fea, xh_list[1], xh_list[2], xfh_list[0], xp_list)
        xhl_new = self.half_lower_infer(h_fea, xh_list[2], xh_list[1], xfh_list[1], xp_list)
