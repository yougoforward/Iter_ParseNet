...
class DecoderModule(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.conv0 = nn.Sequential(nn.Conv2d(48, 512, kernel_size=3, padding=1, dilation=1, bias=False),
                                   BatchNorm2d(512), nn.ReLU(inplace=False))
        self.conv1 = nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, padding=1, dilation=1, bias=False),
                                   BatchNorm2d(256), nn.ReLU(inplace=False))

        self.conv2 = nn.Sequential(nn.Conv2d(24, 48, kernel_size=1, stride=1, padding=0, dilation=1, bias=False),
...
class Decoder(nn.Module):
    def __init__(self, num_classes):
        ...
        self.layer5 = MagicModule(1280, 512, 1)
        self.layer6 = DecoderModule(num_classes)
        self.layerh = AlphaHBDecoder(hbody_cls=3)
        self.layerf = AlphaFBDecoder(fbody_cls=2)

        self.layer_dsn = nn.Sequential(nn.Conv2d(120, 256, kernel_size=3, stride=1, padding=1),
                                       BatchNorm2d(256), nn.ReLU(inplace=False),
...
class OCNet(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        self.encoder = EfficientNet.from_name('efficientnet-b2')
    ...
    def forward(self, x):
        _ , x = self.encoder.extract_features(x)
        x = self.decoder(x)
        return x
