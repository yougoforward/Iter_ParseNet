    ...
    def forward(self, preds, targets):
        ...
        for i in range(len(preds[9])):
            ...
            for j in range(self.num_classes-1):
                ...
                pred_dp = F.interpolate(input=preds[9][i][j], size=(h, w), mode='bilinear', align_corners=True)
        ...
        loss_dsn = self.criterion(pred_dsn, targets[0])
        return loss + 0.4 * loss_hb + 0.4 * loss_fb + \
               0.4*(loss_fh_att + loss_up_att + loss_lp_att + loss_com_att + loss_dp_att/(self.num_classes-1)) + 0.4 * loss_dsn
