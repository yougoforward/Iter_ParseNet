...
class ABRLovaszLoss_List_att(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        pred_dsn = F.interpolate(input=preds[-1], size=(h, w), mode='bilinear', align_corners=True)
        loss_dsn = self.criterion(pred_dsn, targets[0])
        return loss + 0.2 * loss_hb + 0.2 * loss_fb + 0.1*(loss_fh_att+loss_up_att+loss_lp_att)+ 0.4 * loss_dsn
...
class ABRLovaszLoss_List_att_dp(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        dp_loss = sum(dp_loss)*0.5

        # dsn loss
        pred_dsn = F.interpolate(input=preds[-1], size=(h, w), mode='bilinear', align_corners=True)
        loss_dsn = self.criterion(pred_dsn, targets[0])
        return loss + 0.2 * loss_hb + 0.2 * loss_fb + 0.1*(loss_fh_att+loss_up_att+loss_lp_att)+ dp_loss+ 0.4 * loss_dsn
